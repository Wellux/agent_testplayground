...
export class LogView extends ItemView {
  ...
  getViewType(): string { return LOG_VIEW_TYPE; }
  getDisplayText(): string { return "Ralph log"; }
  ...
  async onOpen(): Promise<void> {
    ...
    const refresh = async () => {
      ...
      if (!s.vaultRoot) {
        pre.textContent = "Set vaultRoot in plugin settings.";
        return;
      }
      const logPath = path.join(s.vaultRoot, "90-Meta", "log.md");
      try {
        ...
        pre.textContent = err.code === "ENOENT" ? `(no log yet at ${logPath})` : `error: ${err.message}`;
      }
    };
