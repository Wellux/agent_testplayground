...
export class MetricsView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    root.style.fontFamily = "var(--font-monospace)";
    root.style.fontSize = "12px";

    const refresh = async () => {
      const s = this.getSettings();
      if (!s.vaultPath) {
        root.textContent = "Set vaultPath in plugin settings.";
        return;
      }
      const ndjsonPath = path.join(s.vaultPath, "90-Meta", "metrics.ndjson");
      let lines: string[] = [];
      try {
        ...
        root.empty();
        root.textContent = err.code === "ENOENT"
          ? `(no metrics yet at ${ndjsonPath})`
          : `error: ${err.message}`;
        return;
      }
      ...
      for (const axis of ["research", "memory", "skills", "interaction", "compress"]) {
        const li = ul.createEl("li");
        li.textContent = `${axis}: ${counts.get(axis) ?? 0}`;
      }

      const sparkAxes = ["memory", "skills", "interaction"];
      for (const axis of sparkAxes) {
