...
export class StatusBar {
  ...
  constructor(el: HTMLElement, getSettings: () => RalphSettings) {
    ...
    this.el.style.fontFamily = "var(--font-monospace)";
  ...
  async refresh(): Promise<void> {
    ...
    if (!s.vaultPath) {
      this.el.textContent = "🌀 Ralph · set vaultPath in settings";
      return;
    }
    const stateFile = path.join(s.vaultPath, "90-Meta", "ralph-state.json");
    const stopFile = path.join(s.vaultPath, "90-Meta", "STOP");
